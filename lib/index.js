...
module.exports = global.XPMailer = new XP.Class('XPMailer', {
    ...
    send: {
        ...
        value(mail, attachments, callback) {
            ...
            // Asserting
            if (!XP.isObject(mail)) { callback(XP.error(500, '"mail" must be Object.')); return; }
            if (!XP.isString(mail.to, true)) { callback(XP.error(500, '"mail.to" must be string.')); return; }
            if (!XP.isVoid(mail.from) && !XP.isString(mail.from, true)) { callback(XP.error(500, '"mail.from" must be string.')); return; }
            if (!XP.isVoid(mail.cc) && !XP.isString(mail.cc)) { callback(XP.error(500, '"mail.cc" must be string.')); return; }
            if (!XP.isVoid(mail.bcc) && !XP.isString(mail.bcc)) { callback(XP.error(500, '"mail.cc" must be string.')); return; }
            if (!XP.isVoid(mail.html) && !XP.isString(mail.html)) { callback(XP.error(500, '"mail.html" must be string.')); return; }
            if (!XP.isVoid(mail.text) && !XP.isString(mail.text)) { callback(XP.error(500, '"mail.text" must be string.')); return; }
            if (!XP.isVoid(attachments) && !XP.isArray(attachments)) { callback(XP.error(500, '"attachments" must be Array.')); return; }
            if (!mail.from && !this.noreply) { callback(XP.error(500, '"mail.from" must be string.')); return; }
